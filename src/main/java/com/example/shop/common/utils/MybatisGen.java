...
public class MybatisGen {
    ...
    public static void main(String[] args) {
        String path = System.getProperty("user.dir").concat(File.separator).concat("src").concat(File.separator).concat("main")
                .concat(File.separator).concat("java").concat(File.separator).concat("com").concat(File.separator).concat("example").concat(File.separator).concat("shop").concat(File.separator);
        ...
        FastAutoGenerator.create("jdbc:mysql://localhost:3306/shopping", "root", "121113")
                .globalConfig(builder -> {
                    builder.author("cjn").enableSwagger(); // 设置作者// 开启 swagger 模式
